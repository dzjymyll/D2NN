...
def tf_udft2(self):
    ...
    out = tf.compat.v1.fft2d(self)/math.sqrt(M*N)
    return out
...
def tf_uidft2(self):
    ...
    M,N = self.shape[1],self.shape[2]
    # self = tf.convert_to_tensor(self, dtype=tf.complex64)
    out = tf.compat.v1.ifft2d(self)*math.sqrt(M*N)
    return out
...
#    fy2 = np.outer(np.ones(M.value),(fy)**2)
    if theta0:#BANDLIMIT OF THE FREE-SPACE PROPAGATION
        ...
        Q = tf.compat.v1.to_float(((fx2+fy2)<=(f0**2)))
    else:
        Q = tf.compat.v1.to_float(((fx2+fy2)<=(1/wlengtheff**2)))
